...
Scope = Literal["document", "chapter", "section", "pages", "page"]
BBox = tuple[float, float, float, float]  # đơn vị point (72dpi) như PyMuPDF trả về


@dataclass(frozen=True)
class Block:
    """Một khối văn bản trên trang — đơn vị nhỏ nhất mà một `anchor` neo tới được."""
...
class ScopeContext:
    ...
    # Ngữ cảnh lân cận — chỉ để model hiểu, KHÔNG tóm tắt vào. Luôn rỗng từ khi
    # bỏ scope `selection`: mọi phạm vi còn lại đều là trang trọn vẹn nên không
    # có "lân cận" nào nằm ngoài. Giữ field để `prompts/summarize.v1.md` không
    # phải đổi version cho một biến đã luôn rỗng.
    context_text: str = ""

    # Cùng nội dung với `text` nhưng có chèn nhãn `[trang N]` và `(p06-b03)` để
    # model biết đặt anchor vào đâu. HAI BẢN chứ không một, vì `verify.py` so
    # quote vào `text`: nếu nhãn nằm trong đó thì một trích dẫn vắt qua hai khối
    # sẽ có "(p06-b04)" chen giữa và bị loại oan.
    annotated_text: str = ""
    anchors_available: list[Anchor] | None = None

    @property
    def prompt_text(self) -> str:
        """Bản gửi cho model. Dùng ở `tools/`, không dùng để verify."""
        return self.annotated_text or self.text
